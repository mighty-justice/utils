...
function closestIndexTo (dirtyDateToCompare, dirtyDatesArray) {
  ...
  dirtyDatesArray.forEach(function (dirtyDate, index) {
    ...
    if (result$$1 === undefined || distance < minDistance) {
      result$$1 = index;
...
function buildDistanceInWordsLocale () {
  ...
  function localize (token, count, options) {
    ...
    if (typeof distanceInWordsLocale[token] === 'string') {
      result$$1 = distanceInWordsLocale[token];
    } else if (count === 1) {
      result$$1 = distanceInWordsLocale[token].one;
    } else {
      result$$1 = distanceInWordsLocale[token].other.replace('{{count}}', count);
    }
  ...
  for (key in obj) {
    ...
    if (isOverridePresent) {
      ...
      if (overrides && overrides.length === 2) {
        result$$1[overrides[0]] = overrides[1];
        continue;
      }
    }
...
function domToReact(nodes, options) {
  ...
  for (var i = 0, len = nodes.length; i < len; i++) {
    ...
    result$$1.push(React.createElement(node.name, props, children));
  }

  return result$$1.length === 1 ? result$$1[0] : result$$1;
}
